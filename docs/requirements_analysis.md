...
|---|---|---|
| Q-1 | **Multi-tenant definition:** The brief says "multi-tenant system" — does this mean multiple independent organizations with data isolation, or simply multiple individual GitHub users sharing one deployment? This significantly affects the database schema (tenant ID columns, row-level security) and auth flow. | Architecture & DB design |
